            ...
            for(j = 0; j < blocks.length; j++){
                if(blocks[j].parent !== null){
                    if(blocks[j].parent.touching){
                        blocks[j].content.style.transform =`translate(${getOffset(main).left + getOffset(blocks[j].parent.content).left - /*652*/ 490}px, ${getOffset(main).top + getOffset(blocks[j].parent.content).top + 60}px)`;
                    }else{
                        blocks[j].content.style.transform =`translate(${getOffset(main).left + getOffset(blocks[j].parent.content).left - /*652*/ 490}px, ${getOffset(main).top + getOffset(blocks[j].parent.content).top - 14}px)`;
                    }
                }
            }
        }
